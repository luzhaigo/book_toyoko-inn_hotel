...
async function start() {
  ...
  page.on(
    'console',
    msg => console[msg.type()] && console[msg.type()](msg.text())
  );
  await page.goto(URL);
  ...
  let cssPath = await page.evaluate(() => {
    ...
    for (let i = 0; i < tbodys.length; i++) {
      let tbody = tbodys[i];
      if (
        domTestingUtils.queryByText(tbody, '東橫INN俱樂部會員卡會員雙床房') !==
        null
      ) {
        const bookButtons = domTestingUtils.getAllByText(tbody, '預約');
